...
(defun havannah ()
	(setq computer '())
	(format t "~%If you want for computer to play first please enter 'C': ")
	(setq first (read))
    (if (equal first 'C) (setq computer 't))
	(if computer (progn (setq comp 'X)(format t "Computer is first player ~% ")) (progn (setq comp 'O)(format t "You are first player ~% ")))
	...
	(loop while (not (equal endgame 't)) do 
		...
		;igra kompjuter
		(when (equal (caar player) comp) 
			(cond ((null firstMove)	;Ukoliko je prvi potez
					(setq input (initFirstMove))
					(setq firstMove input)
					(setq lastMove firstMove)
				  )
				(t
					(setq input (cadr (minimax lastMove depth alpha beta 't))) ;za svaki naredni potez se poziva minimax
					(if (equal input endMove) 
						(setq input (list (car input) (1+ (cadr input))))
						(setq lastMove input)
					)
				)
			)
		...
		;igra covek
		(when (not (equal (caar player) comp)) 
			(setq input (read))
			(if (equal input 'Q) 
				(return))
		)
...
(defun insertMove (input player playedMoves) 
	...
	(cond 
			( (equalp input (inputValidate input))	;Proverava validnost upisa
							...
							;Brisemo unetu poziciju iz liste slobodnih pozicija za unos (unentered koristimo za generisanje liste svih slobodnih pozicija za minimax)
							(if (>= (position (car input) letter :test #'equal) dim)
									;implementacija permanent delete f-je
									(setq unentered (substitute (list (car input) (delete (- (cadr input) (- (* 2 dim) (length (car (cdr (assoc (car input) alist)))) )) (cadr(assoc(car input) unentered)))) (assoc (car input) unentered) unentered))
									
									(setq unentered (substitute (list (car input) (delete (1- (cadr input)) (cadr(assoc(car input) unentered))) ) (assoc (car input) unentered) unentered))
							)
			...
			(t		
					(when (equal (caar player) comp) 
						...
						(setq input (cadr (minimax lastMove depth alpha beta 't)))
					)
					(when (not (equal (caar player) comp))					
							(format t "~%Invalid move: ~A. Please enter your move in format (letter number): " input)
							(setq input (read))							
...
(defun evaluate (node maximizingPlayer)
	(if maximizingPlayer
		
		(cond   ( (OR (equal node (list (first letter) (length (cadr (assoc (first letter) alist)))))
						(equal node (list (car (last letter)) (length (cadr (assoc (car (last letter)) alist)))))) 100)
						
				( (equal (car node) (car firstMove)) 60)
				
				( (equal (car node) (nth 1 (member (car firstMove) letter))) 80)
				( (equal (car node) (nth 2 (member (car firstMove) letter))) 50)
				( (equal (car node) (nth 3 (member (car firstMove) letter))) 40)
				( (equal (car node) (nth 4 (member (car firstMove) letter))) 30)
				( (equal (car node) (nth 5 (member (car firstMove) letter))) 20)
				
				( (equal (car node) (nth 1 (member (car firstMove) (reverse letter)))) 80)
				( (equal (car node) (nth 2 (member (car firstMove) (reverse letter)))) 50)
				( (equal (car node) (nth 3 (member (car firstMove) (reverse letter)))) 40)
				( (equal (car node) (nth 4 (member (car firstMove) (reverse letter)))) 30)
				( (equal (car node) (nth 5 (member (car firstMove) (reverse letter)))) 20)
...
(defun minimax (node depth alpha beta maximizingPlayer)
	(cond 
			( (OR (equal depth 0) (equal endgame 'T))
				(list (evaluate bestNode maximizingPlayer) bestNode)
			)
			
			( (equal maximizingPlayer 't) 
				(setq maxEval (list most-negative-fixnum '()))
				(setq x (neighbours node unentered)) ;U x smestamo susede node-a koji su slobodni za unos
				;(setq x (car x))
				(setq bestVal '0)
				(setq bestNode (nth 0 x))
				(loop for i from 1 to (1- (length x)) do ;Prolazimo kroz sve susede node-a
					(when (> (evaluate (nth i x) maximizingPlayer) bestVal)
						(setq bestVal (evaluate (nth i x) maximizingPlayer))
						(setq bestNode (nth i x))
					)	
				)
				(setq eval (minimax bestNode (1- depth) alpha beta '()))
				(setq maxEval (list (max (car maxEval) (car eval)) (cadr eval) ))
				(setq alpha (max alpha (car eval)))
				(if (>= alpha beta) (return))
                maxEval				
			)
			(t 
				(setq minEval (list most-positive-fixnum '()))
				(setq x (neighbours node unentered)) ;U x smestamo susede node-a koji su slobodni za unos
				;(setq x (car x))
				(setq bestVal '0)
				(setq bestNode (nth 0 x))
				...
				(setq eval (minimax bestNode (1- depth) alpha beta 'T))
				(setq minEval (list (min (car minEval) (car eval)) (cadr eval)))
				(setq beta (min beta (car eval)))
				(if (>= alpha beta) (return))
				minEval
			)
	)
